...
# GEO Dataset GSE107942 

Study of Lipoxin Pathways in Mouse Kidney Dataset GSE107942

https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE107942

The overall idea of this analysis is to see which genes or pathways are over/down regulated in the particular mouse models of this dataset, and see whether it is related with Lipoxin Pathways.
...
* Case = DIABETIC LIPOXIN
* Control = DIABETIC VEHICLE

```{r}
datain <- read.delim("GSE107942_lipoxin_gbod2_countmx.txt",header=TRUE,row.names=1)
datain <- cbind(gsub(".*_","",rownames(datain)),datain)
colnames(datain)[1] <- "Gene"
rownames(datain) <- gsub("_.*","",rownames(datain))
datain.bkp <- datain
gene_list <- cbind(rownames(datain),as.character(datain[,1]))
colnames(gene_list) <- c("GeneENS","GeneName")
# gsms <- "XXXXXXXXXXXXX000000XXXXXXXXXXXX111111"
gsms <- "X000000000000000000111111111111111111"
sml <- c()
...
DataGroups <- c(paste("ND-BLP",seq(1:6),sep="_"),paste("ND-VEH",seq(1:6),sep="_"),paste("ND-LPX",seq(1:6),sep="_"),paste("D-BLP",seq(1:6),sep="_"),paste("D-VEH",seq(1:6),sep="_"),paste("D-LPX",seq(1:6),sep="_"))
...
colnames(countdata) <- DataGroups
condition <- factor(c(rep("ND",18),rep("D",18)),levels=c("ND","D"))
coldata <- data.frame(row.names=colnames(countdata), condition)
dds <- DESeqDataSetFromMatrix(countData=countdata, colData=coldata, design=~condition)
dds <- dds[rowSums(fpm(dds)>1)>=6]
dim(dds)
```
...
```{r}
logcounts <- log2(counts(dds,normalized=T) + 1)
...
boxplot(logcounts, 
        ...
        las=2, cex.axis=0.7,
        col=mycols)
legend("topleft", legend=c("DCTR", "DLPX"),
       fill=unique(mycols),
       #lty=1,
...
Remove DLPX6 and NDVEH3!!

```{r}
gsms <- "X00000000X00000000011111111111111111X"
sml <- c()
for (i in 1:nchar(gsms)) { sml[i] <- substr(gsms,i,i) }
sel <- which(sml != "X")
sml <- sml[sel]
datain <-datain.bkp [,sel]
```

```{r}
DataGroups <- DataGroups[-9][-35]
countdata <- as.matrix(datain)
colnames(countdata) <- DataGroups
condition <- factor(c(rep("ND",17),rep("D",17)),levels=c("ND","D"))
coldata <- data.frame(row.names=colnames(countdata), condition, gsub("_.*","",DataGroups))
colnames(coldata) <- c("group","condition")
dds <- DESeqDataSetFromMatrix(countData=countdata, colData=coldata, design=~condition)
dds <- dds[rowSums(fpm(dds)>1)>=6]
dim(dds)
```
...
bbplot$group <- condition
colnames(bbplot)[1] <- "lib.size"
p1 <- ggplot(bbplot,aes(x=ids,y=lib.size*1e-6,fill=group)) + 
    geom_bar(stat = "identity",position = position_dodge()) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.5)) +
    geom_abline(intercept=20, slope=0) +
    ggtitle("Library Size per sample") +
    ...
    theme(legend.text = element_text(size = 10),legend.key.size = unit(0.5, "lines")) +
    scale_fill_manual(values=unique(mycols))
mycols <- rev(hue_pal()(6)[as.numeric(factor(gsub("_.*","",DataGroups)))])
bbplot <- as.data.frame(colSums(assay(dds)))
bbplot$ids <- rownames(bbplot)
bbplot$group <- gsub("_.*","",DataGroups)
colnames(bbplot)[1] <- "lib.size"
p2 <- ggplot(bbplot,aes(x=ids,y=lib.size*1e-6,fill=group)) + 
    geom_bar(stat = "identity",position = position_dodge()) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.5)) +
    geom_abline(intercept=20, slope=0) +
    ggtitle("Library Size per sample") +
...
```
```{r}
mycols6 <- rev(hue_pal()(6)[as.numeric(factor(gsub("_.*","",DataGroups)))])
z <- DESeq2::plotPCA(vst, intgroup="condition",ntop=dim(vst)[1])
z + geom_text(show.legend=F,aes(label = DataGroups),nudge_y=2) + 
   scale_color_manual(values=unique(mycols6))
```
```{r}
mycols2 <- rev(hue_pal()(2)[as.numeric(factor(condition))])
z <- DESeq2::plotPCA(vst, intgroup="group",ntop=dim(vst)[1])
z + geom_text(show.legend=F,aes(label = DataGroups),nudge_y=2) + 
   scale_color_manual(values=unique(mycols2))
```
...
res <- results(dds, contrast=c("condition","D-LPX","D-VEH"),alpha=0.05,lfcThreshold = 0)
table(res$padj<0.05)
res$symbol <- gene_list[match(rownames(res),gene_list[,1]),2]
...
```{r}
anno.neworder <- union(which(colData(vst)[, "condition"] %in% 
                                 c("D-VEH","D-LPX")),c(1:dim(colData(vst))[1]))
...
plt <- ggplot(data, aes(x=condition, y=count, color=condition))+
   geom_boxplot(alpha=0,width=0.1,show.legend=FALSE) +
   #scale_y_log10() +
...
resdata <- merge(as.data.frame(resorder),
                 as.data.frame(counts(dds,normalized=TRUE)
                               [,c(paste("D-LPX",seq(1:5),sep="_"),
                                   paste("D-VEH",seq(1:6),sep="_"))]),by="row.names", sort=FALSE)
...
selGenes <- resdata[order(-abs(resdata$stat)),'GeneENS']
selGenes.names <- selGenes.names[!is.na(selGenes)][c(1:10)]
selGenes <- selGenes[!is.na(selGenes)][c(1:10)]
data<-plotCounts(dds, selGenes[1], "condition",returnData=T)
for (i in selGenes){
  data[,i] <- log2(fpm(dds)[match(i,rownames(res)),])
  data
...
```{r}
write.csv(resdata,'raw_stats_gse107942_dveh-vs-dlpx.csv')
```
...
p3 <- plotEnrichment(pathways.MM.K[[selected_pathways[9]]], ranks) + labs(title=selected_pathways[9])
p3a <- plotEnrichment(pathways.MM.K[[selected_pathways[10]]], ranks) + labs(title=selected_pathways[10])
p3b <- plotEnrichment(pathways.MM.K[[selected_pathways[11]]], ranks) + labs(title=selected_pathways[11])
...
for (p in selected_pathways_ok) {
  ...
  selGenes <- selGenes[order(selGenes$stat),]$GeneENS
  # selGenes <- selGenes[order(selGenes$stat),22]
  ...
  colnames(data) <- c(colnames(data)[1:2],unique(selGenes.names))
  data <- melt(data,id.vars='condition',measure.vars=selGenes.names)
  data <- data[str_detect(data$condition, eval(expression(paste("^D-LPX","^D-VEH",sep="|")))),]

  myplot<-ggplot(data, aes(x=variable, y=value, color=condition)) +
   geom_boxplot(alpha=0.2,width=0.2,show.legend=FALSE,position=position_dodge(0.3)) +
   #scale_y_log10() +
   scale_color_manual(values=rev(unique(mycols2))) +
   geom_point(position=position_dodge(width=0.3))+
   labs(x="Gene",y="CPM (log2 transformed)") +
  # facet_wrap( ~ variable, scales="free") +
...
```{r}
pathway_codes <- c("mmu04064","mmu04668","mmu04151","mmu04630","mmu04150","mmu03320","mmu04072","mmu04080","mmu04218","mmu00590")
```
...
ctrl='dveh'
case='dlpx'
outsuffix=paste0("pathview.",ctrl,"-vs-",case)
for (mypath in pathway_codes) {
  ...
  files <- list.files(pattern = paste0(mypath,".",outsuffix,".png"))
...
```{r comment=""}
res <- results(dds, contrast=c("condition","D-VEH","ND-VEH"),alpha=0.05,lfcThreshold = 0)
res$symbol <- gene_list[match(rownames(res),gene_list[,1]),2]
res$entrezid <- mapIds(org.Mm.eg.db,
                     keys=row.names(res),
                     column="ENTREZID",
...
```{r}
anno.neworder <- union(which(colData(vst)[, "condition"] %in% 
                                 c("D-VEH","ND-VEH")),c(1:dim(colData(vst))[1]))
mat <- assay(vst)[ head(order(res$padj),20), anno.neworder]
mat <- mat - rowMeans(mat)
rownames_mat_match <- gene_list[match(rownames(mat),gene_list[,1]),]
rownames(mat) <- rownames_mat_match[,2]
...
```{r}
write.csv(resdata,'raw_stats_gse107942_ndveh-vs-dveh.csv')
```
...
write.csv(n,'pathway_stats_gse107942_ndveh-vs-dveh.csv')
```
...
```{r comment=""}
res <- results(dds, contrast=c("condition","ND-LPX","ND-VEH"),alpha=0.05,lfcThreshold = 0)
res$symbol <- gene_list[match(rownames(res),gene_list[,1]),2]
res$entrezid <- mapIds(org.Mm.eg.db,
                     keys=row.names(res),
                     column="ENTREZID",
...
```{r}
write.csv(resdata,'raw_stats_gse107942_ndveh-vs-ndlpx.csv')
```
...
write.csv(n,'pathway_stats_gse107942_ndveh-vs-ndlpx.csv')
```
...
```{r comment=""}
res <- results(dds, contrast=c("condition","D-LPX","ND-LPX"),alpha=0.05,lfcThreshold = 0)
res$symbol <- gene_list[match(rownames(res),gene_list[,1]),2]
res$entrezid <- mapIds(org.Mm.eg.db,
                     keys=row.names(res),
                     column="ENTREZID",
...
```{r}
anno.neworder <- union(which(colData(vst)[, "condition"] %in% 
                                 c("D-LPX","ND-LPX")),c(1:dim(colData(vst))[1]))
mat <- assay(vst)[ head(order(res$padj),20), anno.neworder]
mat <- mat - rowMeans(mat)
rownames_mat_match <- gene_list[match(rownames(mat),gene_list[,1]),]
rownames(mat) <- rownames_mat_match[,2]
...
```{r}
write.csv(resdata,'raw_stats_gse107942_ndlpx-vs-dlpx.csv')
```
...
write.csv(n,'pathway_stats_gse107942_ndlpx-vs-dlpx.csv')
```
