...
```{r plots-igf1r-ntg, warning=F}
run_all_process("IGF1R","Ntg")
```


```{r plots-capi3k-ntg, warning=F}
run_all_process("caPI3K","Ntg")
```


```{r plots-dnpi3k-ntg, warning=F}
run_all_process("dnPI3K","Ntg")
```


```{r plots-swim-nonswim, warning=F}
run_all_process("SWIM","NONSWIM")
```


```{r plots-tacsev-sham, warning=F}
run_all_process("TACSEV","Sham")
```


```{r plots-tacmod-sham, warning=F}
run_all_process("TACMOD","Sham")
```


```{r plots-igf1r-sham, warning=F}
run_all_process("IGF1R","Sham")
```


```{r plots-igf1r-nonswim, warning=F}
run_all_process("IGF1R","NONSWIM")
```

```{r plots-capi3k-dnpi3k, warning=F}
run_all_process("caPI3K","dnPI3K")
```


```{r plots-capi3k-igf1r, warning=F}
run_all_process("caPI3K","IGF1R")
```


```{r plots-capi3k-swim, warning=F}
run_all_process("caPI3K","SWIM")
```


```{r plots-dnpi3k-igf1r, warning=F}
run_all_process("dnPI3K","IGF1R")
```


```{r plots-igf1r-swim, warning=F}
run_all_process("IGF1R","SWIM")
```


```{r plots-tacsev-tacmod, warning=F}
run_all_process("TACSEV","TACMOD")
```


```{r plots-ntg-nonswim, warning=F}
run_all_process("Ntg","NONSWIM")
```


```{r plots-ntg-sham, warning=F}
...
```{r feelnc-pairs-test1}
ggplot(data, aes(x=symbol, y=value, color=condition, shape=type)) +
...
run_all_feelnc <- function (case,control,gene_matrix=genes){
  ...
  data$symbol <- factor(data$symbol,levels = unique(data$symbol))
   ...
   p <- ggplot(data, aes(x=symbol, y=value, color=condition, shape=type)) +
     geom_boxplot(alpha=0.2,width=0.2,show.legend=FALSE,position=position_dodge(0.3)) +
     scale_color_manual(values=unique(mycols[data$condition])) +
     geom_point(position=position_dodge(width=0.3),size=2.5)+
     labs(x="Gene",y="CPM (log2 transformed)", shape="Gene Type",color="Sample") +
