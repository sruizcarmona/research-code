...
dirs_raw <- list.dirs("PRO_HEART/ANTWERPEN_PH")
dirs <- dirs_raw[str_detect(dirs_raw,"YEAR")]
...
```{r}
# test <- paste0(paste0(dirs[grep("liam",tolower(dirs))],"/09/"),list.files(paste0(dirs[grep("liam",tolower(dirs))],"/09")))
test <- paste0(dirs[grep("DORIAN",dirs)],"/",list.files(dirs[grep("DORIAN",dirs)]))[1:3]
```
...
```{r}
athlete.predictors <- calc_predictors(test)
athlete.predictors
```
