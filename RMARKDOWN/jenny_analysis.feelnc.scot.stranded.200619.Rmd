...
```{r library-size-per-sample-group1}
# bplot <- fcounts.ensembl$samples[c(11,22,30:36,1:10,12:21,23:29),]
# bplot$ids <- str_extract(targets$sample,"JM[0-9]{1,2}")
# bplot$ids <- factor(paste0("JM",c(1:36)),levels=paste0("JM",c(1:36)))
# bplot$group <- factor(as.character(bplot$group),levels=unique(bplot$group))
...
```{r library-size-per-sample-group2}
# bplot <- fcounts.ensembl$samples[c(11,22,30:36,1:10,12:21,23:29),]
# bplot$ids <- str_extract(targets$sample,"JM[0-9]{1,2}")
# bplot$ids <- factor(paste0("JM",c(1:36)),levels=paste0("JM",c(1:36)))
# bplot$group <- factor(as.character(bplot$group),levels=unique(bplot$group))
...
run_all_process("IGF1R","Ntg",volcano_x = 5, volcano_y = 40)
```


```{r plots-capi3k-ntg, warning=F}
run_all_process("caPI3K","Ntg",volcano_x = 5, volcano_y = 20)
```


```{r plots-dnpi3k-ntg, warning=F}
run_all_process("dnPI3K","Ntg",volcano_x = 7, volcano_y = 40)
```


```{r plots-swim-nonswim, warning=F}
run_all_process("SWIM","NONSWIM",volcano_x = 5, volcano_y = 15)
```


```{r plots-tacsev-sham, warning=F}
run_all_process("TACSEV","Sham",volcano_x = 5, volcano_y = 30)
```


```{r plots-tacmod-sham, warning=F}
run_all_process("TACMOD","Sham",volcano_x = 5, volcano_y = 8)
```


```{r plots-igf1r-sham, warning=F}
run_all_process("IGF1R","Sham",volcano_x = 7, volcano_y = 40)
```


```{r plots-igf1r-nonswim, warning=F}
run_all_process("IGF1R","NONSWIM",volcano_x = 6, volcano_y = 40)
```

```{r plots-capi3k-dnpi3k, warning=F}
run_all_process("caPI3K","dnPI3K",volcano_x = 6, volcano_y = 40)
```


```{r plots-capi3k-igf1r, warning=F}
run_all_process("caPI3K","IGF1R",volcano_x = 7, volcano_y = 30)
```


```{r plots-capi3k-swim, warning=F}
run_all_process("caPI3K","SWIM",volcano_x = 15, volcano_y = 25)
```


```{r plots-dnpi3k-igf1r, warning=F}
run_all_process("dnPI3K","IGF1R",volcano_x = 8, volcano_y = 50)
```


```{r plots-igf1r-swim, warning=F}
run_all_process("IGF1R","SWIM",volcano_x = 7, volcano_y = 40)
```


```{r plots-tacsev-tacmod, warning=F}
run_all_process("TACSEV","TACMOD",volcano_x = 5, volcano_y = 15)
```


```{r plots-ntg-nonswim, warning=F}
run_all_process("Ntg","NONSWIM",volcano_x = 5, volcano_y = 7)
```


```{r plots-ntg-sham, warning=F}
run_all_process("Ntg","Sham",volcano_x = 7, volcano_y = 7)
```
...
feelnc.pairs <- feelnc.pairs[,c(1,2,3,4,11,5,6,7,8,9,10)]
