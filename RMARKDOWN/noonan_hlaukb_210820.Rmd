...
subtitle: "Jonathan Noonan, Initial report"
...
### Categorical models {.tabset}

#### Model 1 (only HLA-A)

```{r}
test_hlaa <- hla_ihd_q1 %>% 
  select(eid, contains(c("HLA-A")), ihd_group) %>% 
  na.omit()
test_hlaa <- test_hlaa %>% 
  mutate(eid = as.character(eid)) %>% 
  pivot_longer(-c(ihd_group, eid)) %>%
  # pivot_longer(c(`HLA-A_1`,`HLA-A_2`))
  separate(name, into = c("name", NA), sep = "_") %>% 
  mutate(value = factor(paste0(name, "*", value)),
         ihd_group = relevel(ihd_group, ref = "Died, never had IHD")
         )

# xtabs(~`HLA-A_1` + ihd_group, data = hla_ihd_q1)
# xtabs(~ihd_group + value, data = test_hlaa)
```

```{r}
# kk <- test_hlaa %>% select(ihd_group, value)
logit1 <- glm(ihd_group ~ value, family = binomial, data = test_hlaa)
# logitkk <- glm(ihd_group ~ ., family = binomial, data = kk)
# summary(logit1)
# summary(logitkk)
# coef(logit1) %>% exp()
# logit1$aic
...
```{r include=F}
# logit2 <- MASS::stepAIC(logit1)
# summary(logit2)
# summary(logit1$fitted.values)
# # cbind(test_hlaa, k = round(logit1$fitted.values,2)) %>% arrange(-k)
# logit2$aic
```

```{r message=FALSE, fig.width=5, fig.height=5}
logit_res <- cbind(test_hlaa, pred = round(logit1$fitted.values,2)) %>% relocate(pred, .after = ihd_group)
roc_data <- pROC::roc(ihd_group~pred, data = logit_res, plot = TRUE, main = "Test only HLA-A", col= "blue", print.auc = T)
```

#### Model 2 (HLA-A and HLA-B)

```{r}
test_hlab <- hla_ihd_q1 %>% 
  select(eid, contains(c("HLA-A", "HLA-B")), ihd_group) %>% 
  na.omit()
test_hlab <- test_hlab %>% 
  mutate(eid = as.character(eid)) %>% 
  pivot_longer(-c(ihd_group, eid)) %>%
  # pivot_longer(c(`HLA-A_1`,`HLA-A_2`)) %>% 
  separate(name, into = c("name", "ale"), sep = "_") %>% 
  mutate(value = factor(paste0(name, "*", value)),
         ihd_group = relevel(ihd_group, ref = "Died, never had IHD"),
         ) %>% 
...
# xtabs(~`HLA-A_1` + ihd_group, data = hla_ihd_q1)
# xtabs(~ihd_group + `HLA-B`, data = test_hlab)
```

```{r}
logit1 <- glm(ihd_group ~ `HLA-A` + `HLA-B`, family = binomial, data = test_hlab)
# summary(logit1)
# coef(logit1) %>% exp()
# logit1$aic
# confint(logit1) %>% exp()
...
# cbind(test_hlab, k = round(logit1$fitted.values,2)) %>% arrange(-k)
# logit2$aic
```

```{r message=FALSE, fig.width=5, fig.height=5}
logit_res <- cbind(test_hlab, pred = round(logit1$fitted.values,2)) %>% relocate(pred, .after = ihd_group)
roc_data <- pROC::roc(ihd_group~pred, data = logit_res, plot = TRUE, main = "Test HLA-A and HLA-B", col= "blue", print.auc = T, legacy.axes = T)
```

#### Model 3 (All loci) {.active}

```{r}
logreg_data <- hla_ihd_q1 %>% 
  select(eid, starts_with("HLA-"), ihd_group)
...
logreg_data <- logreg_data %>% 
  ...
  pivot_wider(names_from = name, values_from = value) %>% 
  select(ihd_group, starts_with("HLA-"))
...
```{r}
logit_all <- glm(ihd_group ~ ., family = binomial, data = logreg_data)
# summary(logit_all)
# coef(logit_all) %>% exp()
# logit_all$aic
# confint(logit1) %>% exp()
...
```{r message=FALSE, fig.width=5, fig.height=5}
logit_res <- cbind(logreg_data, pred = round(logit_all$fitted.values,2)) %>% relocate(pred, .after = ihd_group)
par(pty = "s")
...
<!-- ```{r} -->
<!-- multi1 <- multinom(ihd_group ~ value, data = test_hlaa) -->
<!-- # summary(multi1) -->
<!-- exp(coef(multi1)) %>%  -->
<!--   sort(decreasing = T) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- foreign::read.dta("https://stats.idre.ucla.edu/stat/data/hsbdemo.dta") -->
<!-- ``` -->


<!-- ```{r} -->
<!-- # example from https://www.biostars.org/p/408461/ in HLA snps -->
<!-- kk <- tribble( -->
...
<!--   1   ,      0    ,     1    ,     0    ,     1, -->
<!-- ) -->
<!-- m1 <- glm( DX ~ DRB1.0401 , family="binomial", data=kk ) -->
<!-- summary(m1) -->
<!-- coef(m1) -->
<!-- confint(m1) -->
<!-- ``` -->
...
<!--    family="binomial", data=kk )  -->
<!-- null<- glm( DX ~ 1 , family="binomial", data=kk )  -->
<!-- summary(full) -->
<!-- summary(null) -->
...
<!--   pivot_longer(-DX) %>%  -->
<!--   filter(value != 0) %>%  -->
<!--   separate(name, into=c("name", "DRB1")) %>%  -->
<!--   uncount(value) %>%  -->
<!--   select(-name) -->
<!-- kk_long -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- full_long <- glm( DX ~ DRB1,  -->
<!--    family="binomial", data=kk_long )  -->
<!-- # null<- glm( DX ~ 1 , family="binomial", data=kk )  -->
<!-- summary(full_long) -->
<!-- summary(full) -->
<!-- coef(full_long) -->
<!-- coef(full) -->
<!-- # anova( null, full , test="Chisq") -->
<!-- cbind(kk, pred = round(full$fitted.values,2)) -->
<!-- cbind(kk_long, pred = round(full_long$fitted.values,2)) -->
<!-- ``` -->

### Numerical models {.tabset}

:::update
For this new models, we changed the categorical definition of each locus to a "count" of each particular HLA type. So each individual with HLA-A\*101 will have a variable called HLA-A\*101 with values 0, 1 or 2 depending on the number of alleles for that particular type. 

In contrast, the categorial type was **wrongly** considering each alelle as different individuals.
:::
...
hla_ihd_q1_012 <- inner_join(ihd_dead_inds %>% select(eid, ihd_group), hla_info_012, by="eid")
```
...
```{r}
test_hlaa <- hla_ihd_q1_012 %>% 
  select(ihd_group, contains(c("HLA.A")), -contains("_NA") ) %>% 
...
```{r}
# kk <- test_hlaa %>% select(ihd_group, value)
logit1 <- glm(ihd_group ~ ., family = binomial, data = test_hlaa)
# logit1_null <- glm(ihd_group ~ 1, family = binomial, data = test_hlaa)
# summary(logit1)
...
```{r message=FALSE, fig.width=5, fig.height=5}
logit1_res <- cbind(test_hlaa, pred = round(logit1$fitted.values,2)) %>% relocate(pred, .after = ihd_group)
roc_data <- pROC::roc(ihd_group~pred, data = logit1_res, plot = TRUE, main = "Test HLA-A", col= "blue", print.auc = T, legacy.axes = T)
```

#### Model 2 (HLA-A and HLA-B)

```{r}
test_hlaab <- hla_ihd_q1_012 %>% 
  select(ihd_group, contains(c("HLA.A", "HLA.B")), -contains("_NA") ) %>% 
  mutate(ihd_group = relevel(ihd_group, ref = "Died, never had IHD")) %>% 
  select(where(~ any(. != 0)))
```

```{r}
logit1 <- glm(ihd_group ~ ., family = binomial, data = test_hlaab)
# logit1_null <- glm(ihd_group ~ 1, family = binomial, data = test_hlaab)
# 
# summary(logit1)
# anova(logit1_null, logit1, test="Chisq")
...
```{r message=FALSE, fig.width=5, fig.height=5}
logit1_res <- cbind(test_hlaab, pred = round(logit1$fitted.values,2)) %>% relocate(pred, .after = ihd_group)
roc_data <- pROC::roc(ihd_group~pred, data = logit1_res, plot = TRUE, main = "Test HLA-A and HLA-B", col= "blue", print.auc = T, legacy.axes = T)
```
...
roc_data <- pROC::roc(ihd_group~pred, data = logit1_res, plot = TRUE, main = "Test all HLA loci", col= "blue", print.auc = T, legacy.axes = T)
```
...
roc_data <- pROC::roc(ihd_group~pred, data = logit1_res, plot = TRUE, main = "Final model (all clean loci)",
                      col= "blue", print.auc = T, legacy.axes = T)
```
