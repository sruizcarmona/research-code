#! /usr/bin/python
# 
# Substitute for rbtether of RDock. Will align input molecules to a reference fragment defined by a smarts string, put a TETHERED ATOM property field
# that rdock will take to restrain positions in the docking process.
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 5:
        sys.exit("USAGE: python tetherSDF.py reference.sdf input.sdf output.sdf 'SMARTS'")
	...
	if numats > 55 and numrot > 7: 
		genconf = 100
		geomopt = 2000
        print "\tDoing conformational search with RandomRotorSearch (%i, %i)..."%(genconf, geomopt),
#        ff.RandomRotorSearch(genconf,geomopt)
