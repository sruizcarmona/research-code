...
os.system("python $SCRIPTS/createGridIn.py -g "+dudsys+"_grid.zip -r /xpeople/sruiz/DUD/"+dudsys+"/glide/"+dudsys+"_rec_grid.maegz -f "+dudsys+"_grid.in -c \""+str('%.5f' % center[0])+" "+str('%.5f' % center[1])+" "+str('%.5f' % center[2])+"\" -x "+str('%.5f' % dists[0])+" -y "+str('%.5f' % dists[1])+" -z "+str('%.5f' % dists[2]))
